...
│   │   ├── dogState.ts       상태 머신 (idle/looking/petting/happy/sleepy)
│   │   ├── dogBehavior.ts    상태별 몸짓 규칙표(호흡·꼬리·귀·표정·문구)
│   │   ├── dogInteraction.ts 쓰다듬기 판정 (누름 + 이동거리)
│   │   ├── dogAnimation.ts   프레임 보간
│   │   ├── dogRender.ts      숫자 → SVG 속성값
│   │   ├── petZones.ts       부위별 쓰다듬기 영역
...
| | Mobile | Desktop |
|---|---|---|
| 상단 | 상태 · 오늘의 쓰담 시간 | 브랜드 + 상태 · 사운드 토글 |
| 강아지 | `max-h: min(52vh, 460px)` | `max-h: min(56vh, 560px)` |
| 안내 문구 | 15px | 17px |
| 진행 바 | 140px | 168px |
| 하단 | (상단 타이머로 대체) | 오늘의 쓰담 |
| 커서 | 손끝 글로우 | 발바닥 커서 + 글로우 |
